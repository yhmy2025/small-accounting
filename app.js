...
function ia(){var d=new Date().toISOString().slice(0,10);var html='<div class=tt id=at><div class="o on" onclick=sa("i") style=flex:1;text-align:center;padding:8px;font-size:14px;cursor:pointer;background:#4f46e5;color:#fff;border-radius:8px>💰 收入</div><div class=o onclick=sa("e") style=flex:1;text-align:center;padding:8px;font-size:14px;cursor:pointer>💸 支出</div></div><div class=fg><label>日期</label><input id=ad type=date value="'+d+'"></div><div class=fg><label>金额（元）</label><input id=aa type=number step=0.01 placeholder="输入金额"></div><div class=qa id=aq></div><div id=ac></div><div class=fg><label>备注</label><input id=an placeholder="选填"></div><button class="btn btn-p" onclick=sr()>💾 保存</button>';$('p-add').innerHTML=html;sc='';cur='i';sa('i')}function sr(){var a=Math.round(parseFloat($('aa').value)*100);if(!a||a<=0){ts('请输入金额');return}if(!sc){ts('请选择类别');return}ld();d.r.push({id:id(),d:$('ad').value,t:cur,c:sc,a:a,n:$('an').value.trim()||'',src:'手动',ts:Date.now()});sv();ts('✅ 保存成功');setTimeout(function(){gt('home')},400)}
...
function lset(){ld();var sn=d.s.name||'';var si=d.s.industry||'';var h='<h2>⚙️ 设置</h2><div class=fg><label>企业名称</label><input id=sn value="'+sn+'"></div><div class=fg><label>行业</label><select id=si><option value="">未选择</option><option value="餐饮"'+(si==="餐饮"?' selected':'')+'>餐饮</option><option value="零售"'+(si==="零售"?' selected':'')+'>零售</option><option value="服务"'+(si==="服务"?' selected':'')+'>服务</option><option value="制造"'+(si==="制造"?' selected':'')+'>制造</option><option value="电商"'+(si==="电商"?' selected':'')+'>电商</option></select></div><button class="btn btn-p" onclick=svset()>💾 保存</button><hr><button class="btn btn-o" onclick=expAll()>📤导出</button><button class="btn btn-o" onclick=clr()>🗑️清除</button>';$('p-set').innerHTML=h}function svset(){ld();d.s.name=$('set-name').value;d.s.industry=$('set-ind').value;sv();ts('✅ 已保存')}
...
var curT="i",curC="";
function rl(){ld();var n=new Date();var ym=n.getFullYear()+"-"+String(n.getMonth()+1).padStart(2,"0");var h="<h2 style=font-size:16px;margin-bottom:10px>\ud83d\udccb \u6d41\u6c34\u660e\u7ec6</h2>"+"<div class=cd style=padding:8px><div style=display:flex;gap:6px;margin-bottom:8px>"+"<input type=month id=lm value='"+ym+"' style=flex:1;padding:8px;border:1px solid #ddd;border-radius:8px;font-size:12px onchange=rl()>"+"<input type=text id=lq placeholder=\u641c\u7d22 style=flex:1;padding:8px;border:1px solid #ddd;border-radius:8px;font-size:12px oninput=rl()>"+"<button class=btn-s onclick=expCSV()>\ud83d\udce4CSV</button></div>";var mi=$("lm");var lm=mi?mi.value:ym;var recs=mr(lm);var q=($("lq")?$("lq").value:"").toLowerCase();if(q)recs=recs.filter(function(r){return(r.c||"").toLowerCase().includes(q)||(r.n||"").toLowerCase().includes(q)});if(recs.length===0){h+="<div class=emp><div class=ic>\ud83d\udccb</div><div>\u6682\u65e0\u8bb0\u5f55</div></div>"}else{h+="<div style=overflow:auto><table><tr><th>\u65e5\u671f<th>\u7c7b\u578b<th>\u7c7b\u522b<th>\u91d1\u989d<th></tr>";for(var i=0;i<recs.length;i++){var r=recs[i];h+="<tr onclick=sd('"+r.id+"')><td>"+r.d.slice(5)+"</td><td><span class=tag "+r.t+">"+(r.t==="i"?"\u6536\u5165":"\u652f\u51fa")+"</span></td><td>"+r.c+"</td><td style=color:"+(r.t==="i"?"#22c55e":"#ef4444")+";font-weight:600>"+(r.t==="i"?"+":"-")+(r.a/100).toFixed(2)+"</td><td>\ud83d\udccb</td></tr>"}h+="</table></div>"}h+="</div>";$("p-list").innerHTML=h}
